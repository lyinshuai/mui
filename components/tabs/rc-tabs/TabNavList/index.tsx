...
import ArrowLeftOutlined from '../../../_assets/ArrowLeftOutlined';
import ArrowRightOutlined from '../../../_assets/ArrowRightOutlined';
import AddButton from './AddButton';
...
function TabNavList(props: TabNavListProps, ref: React.Ref<HTMLDivElement>) {
  ...
  // =================== Arrow & Operations ===================
  const tabsIndexMaps = useMemo(() => {
    const indexMap = tabs.reduce(
      (res, item, index) => {
        if (item.key) {
          res.index[index] = item.key;
          res.key[item.key] = index;
        }
        return res;
      },
      {
  ...
  const toggleTab = (offsetNum: number) => {
    const activeIndex = tabsIndexMaps.key[activeKey];
    let nextIndex = activeIndex + offsetNum;
    if (nextIndex >= tabs.length) {
      nextIndex = activeIndex;
    } else if (nextIndex <= 0) {
      nextIndex = 0;
    }
    const nextKey = tabsIndexMaps.index[nextIndex];
    onTabClick?.(nextKey);
  };
  ...
  return (
    ...
    >
      ...
      {scrollType === 'arrow' && hasDropdown && (
        <ArrowLeftOutlined
          onClick={() => toggleTab(-1)}
          className={classNames(`${prefixCls}-left-icon`, {
            [`${prefixCls}-nav-icon-disabled`]: !startHiddenTabs.length,
          })}
        />
      )}
        ...
        <>
          <ArrowRightOutlined
            ...
            className={classNames(`${prefixCls}-right-icon`, {
              [`${prefixCls}-nav-icon-disabled`]: !endHiddenTabs.length,
            })}
          />
