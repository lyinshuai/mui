...
describe('Select', () => {
  ...
  function toggleOpen(wrapper) {
    act(() => {
      wrapper.find('.m-design-select-selector').simulate('mousedown');
      jest.runAllTimers();
    });
    wrapper.update();
  }
  ...
  it('should have default notFoundContent', () => {
    ...
    toggleOpen(wrapper);
    expect(wrapper.find('.m-design-select-item-option').length).toBeFalsy();
    expect(wrapper.find('.m-design-empty').length).toBeTruthy();
  });
  ...
  it('should not have notFoundContent when mode is combobox and notFoundContent is set', () => {
    ...
    expect(dropdownWrapper.find('.m-design-select-item-option').length).toBeFalsy();
    expect(dropdownWrapper.find('.m-design-select-item-empty').at(0).text()).toBe('not at all');
  });
  ...
  it('should show search icon when showSearch and open', () => {
    ...
    expect(wrapper.find('.mui-down-icon').length).toBe(1);
    expect(wrapper.find('.anticon-search').length).toBe(0);
    wrapper.setProps({ open: true });
    expect(wrapper.find('.mui-down-icon').length).toBe(0);
