...
describe('Result', () => {
  ...
  it('🙂  successPercent should decide the progress status when it exists', () => {
    ...
    expect(wrapper.find('.m-design-result-success')).toHaveLength(1);
  });

  it('🙂  different status, different class', () => {
    const wrapper = mount(<Result status="warning" />);
    expect(wrapper.find('.m-design-result-warning')).toHaveLength(1);

    wrapper.setProps({
      status: 'error',
    });

    expect(wrapper.find('.m-design-result-error')).toHaveLength(1);

    wrapper.setProps({
      status: '500',
    });

    expect(wrapper.find('.m-design-result-500')).toHaveLength(1);

    wrapper.setProps({
      status: 'loading',
    });

    expect(wrapper.find('.m-design-result-loading')).toHaveLength(1);
  });

  it('🙂  When status = 404, the icon is an image', () => {
    const wrapper = mount(<Result status="404" />);
    expect(wrapper.find('.m-design-result-404 .m-design-result-image')).toHaveLength(1);
  });

  it('🙂  When extra is undefined, the extra dom is undefined', () => {
    const wrapper = mount(<Result status="404" />);
    expect(wrapper.find('.m-design-result-extra')).toHaveLength(0);
  });

  it('🙂  result should support className', () => {
    const wrapper = mount(<Result status="404" title="404" className="my-result" />);
    expect(wrapper.find('.m-design-result.my-result')).toHaveLength(1);
  });
