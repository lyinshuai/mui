...
describe('CheckboxGroup', () => {
  ...
  it('should work basically', () => {
    ...
    wrapper.find('.m-design-checkbox-input').at(0).simulate('change');
    expect(onChange).toHaveBeenCalledWith(['Apple']);
    wrapper.find('.m-design-checkbox-input').at(1).simulate('change');
    expect(onChange).toHaveBeenCalledWith(['Apple', 'Pear']);
    wrapper.find('.m-design-checkbox-input').at(2).simulate('change');
  ...
  it('does not trigger onChange callback of both Checkbox and CheckboxGroup when CheckboxGroup is disabled', () => {
    ...
    groupWrapper.find('.m-design-checkbox-input').at(0).simulate('change');
    expect(onChangeGroup).not.toHaveBeenCalled();
    groupWrapper.find('.m-design-checkbox-input').at(1).simulate('change');
    expect(onChangeGroup).not.toHaveBeenCalled();
  });
  ...
  it('should be controlled by value', () => {
    ...
    expect(wrapper.find('.m-design-checkbox-checked').length).toBe(0);
    wrapper.setProps({ value: ['Apple'] });
    wrapper.update();
    expect(wrapper.find('.m-design-checkbox-checked').length).toBe(1);
  });
  ...
  it('checkbox should register value again after value changed', () => {
    ...
    expect(wrapper.find('.m-design-checkbox-input').at(0).prop('checked')).toBe(false);
  });
  ...
  it('should work when checkbox is wrapped by other components', () => {
    ...
    wrapper.find('.m-design-collapse-item').at(0).find('.m-design-collapse-header').simulate('click');
    wrapper.find('.m-design-checkbox-input').at(0).simulate('change');
    expect(wrapper.find('.m-design-checkbox-checked').length).toBe(1);
    wrapper.find('.m-design-checkbox-input').at(0).simulate('change');
    expect(wrapper.find('.m-design-checkbox-checked').length).toBe(0);
  ...
  it('should store latest checkbox value if changed', () => {
    ...
    wrapper.find('.m-design-checkbox-input').first().simulate('change');
