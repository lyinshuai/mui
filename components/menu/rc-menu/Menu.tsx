...
export interface MenuProps
  ...
  onInlineCollapsedChange?: (collapse: boolean) => void;
  ...
  // >>>>> Internal
  /**
   * @private Only Used for `pro-layout`. Do not use in your prod directly and we do not promise any
...
const Menu = React.forwardRef<MenuRef, MenuProps>((props, ref) => {
  ...
  const [inlineCollapsedLocal, setInlineCollapsedLocal] = React.useState(inlineCollapsed);

  React.useEffect(() => {
    if (inlineCollapsed === inlineCollapsedLocal) return;
    setInlineCollapsedLocal(inlineCollapsed);
  }, [inlineCollapsed]);

  const handleCollapsedClick = event => {
    event.stopPropagation();
    const collapsed = !inlineCollapsedLocal;
    setInlineCollapsedLocal(collapsed);
    onInlineCollapsedChange && onInlineCollapsedChange(collapsed);
  };

  // ========================= Mode =========================
  const [mergedMode, mergedInlineCollapsed] = React.useMemo<[MenuMode, boolean]>(() => {
    if ((['inline', 'vertical', 'vertical-left', 'vertical-right']) && inlineCollapsedLocal) {
      return ['vertical', inlineCollapsedLocal];
    }
    return [mode, false];
  }, [mode, inlineCollapsedLocal]);

  // ====================== Responsive ======================
  const [lastVisibleIndex, setLastVisibleIndex] = React.useState(0);
  const allVisible =
    lastVisibleIndex >= childList.length - 1 || mergedMode !== 'horizontal' || disabledOverflow;
    ...
    <Overflow
      ...
      className={classNames(
        ...
        className,
        {
          ...
          [`${prefixCls}-vertical-hover`]: mode === 'vertical',
          [`${prefixCls}-has-collapse-button`]: hasCollapseButton,
          [`${prefixCls}-rtl`]: isRtl,
        },
        rootClassName,
      )}
      suffix={
        hasCollapseButton && (
          <div className={`${prefixCls}-collapse-button-bg`} onClick={(event) => event.stopPropagation()}>
            <div className={`${prefixCls}-collapse-button`} onClick={handleCollapsedClick}>
              {React.createElement(inlineCollapsedLocal ? MenuUnfoldOutlined : MenuFoldOutlined)}
            </div>
          </div>
        )
      }
      ...
      maxCount={
        mergedMode !== 'horizontal' || disabledOverflow ? Overflow.INVALIDATE : Overflow.RESPONSIVE
      }
      ssr="full"
      data-menu-list
