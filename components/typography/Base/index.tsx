...
import CopyOutlined from '../../_assets/CopyOutlined';
import TickOutlined from '../../_assets/TickOutlined';
import EditOutlined from '../../_assets/EditOutlined';
...
const Base = React.forwardRef((props: InternalBlockProps, ref: any) => {
  ...
  const editableRef = React.useRef<{ value: string }>(null);
  ...
  const [textOffsetWidth, setTextOffsetWidth] = React.useState<number | undefined>();
  ...
  React.useEffect(() => {
    ...
    if (editConfig.mode === 'popup') {
      ...
      if (textEle?.innerText) {
        textWidth = getTextWidth(textEle?.innerText);
      }
      if (textWidth && textEle?.offsetWidth) {
        textWidth = Math.min(textWidth, textEle?.offsetWidth);
      }
      setTextOffsetWidth(textWidth);
    }
  ...
  // =========================== Render ===========================
  // >>>>>>>>>>> Editing input
  if (editing && editConfig.mode !== 'popup') {
    return (
      <Editable
  ...
  const renderDisplay = (displayContent: any) => {
    if (editConfig.mode !== 'popup') {
      return displayContent;
    }

    // eslint-disable-next-line no-nested-ternary
    let popupEditorWidth = textOffsetWidth
      ? textOffsetWidth > 800
        ? textOffsetWidth - 93
        : textOffsetWidth
      : 0;
    popupEditorWidth = popupEditorWidth <= 0 ? 0 : popupEditorWidth;
    ...
    const popupContent = (
      <>
        <Editable
          ...
          autoSize={editConfig.autoSize}
          enterIcon={editConfig.enterIcon}
          style={{ width: popupEditorWidth ? `${popupEditorWidth}px` : undefined, ...style }}
        />
        <div className={`${prefixCls}-popup-operation`}>
          <Button
            type="link"
            size="small"
            onClick={() => {
              onEditChange?.(editableRef.current?.value || '');
              editConfig?.onEnd?.();
            }}
          >
            保存
    ...
    return (
      <Popover
        ...
        title={false}
        overlayClassName={`${prefixCls}-popup`}
        trigger="click"
  ...
  const renderCopy = () => {
    ...
    return (
      <Tooltip key="copy" title={copyTitle}>
        ...
        >
          {copied
            ? getNode(iconNodes[1], <TickOutlined />, true)
