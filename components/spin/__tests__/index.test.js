...
describe('Spin', () => {
  ...
  it('should only affect the spin element when set style to a nested <Spin>xx</Spin>', () => {
    ...
    expect(wrapper.find('.m-design-spin-nested-loading').at(0).prop('style')).toBeFalsy();
    expect(wrapper.find('.m-design-spin').at(0).prop('style').background).toBe('red');
  });
  ...
  it('should render 0', () => {
    ...
    expect(wrapper.find('.m-design-spin-container').at(0).text()).toBe('0');
  });

  it('test type dot Spin', () => {
    const wrapper = mount(
      <Spin type="dot" />,
    );
    expect(wrapper.find('.m-design-spin-dot-item')).toHaveLength(4);
  });
});
