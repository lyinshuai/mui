...
const getBarLoading = (prefixCls: string, dotClassName: string) => (
  <LoadingIcon className={classNames(dotClassName, `${prefixCls}-dot-spin`)} spin={false} />
);

const getDotLoading = (prefixCls: string, dotClassName: string) => (
...
function renderIndicator(prefixCls: string, props: SpinProps): React.ReactNode {
  ...
  const getLoading = props.type ? typeMap[props.type] : typeMap.bar;

  return getLoading(prefixCls, dotClassName);
}
