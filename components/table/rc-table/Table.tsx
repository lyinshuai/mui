...
function Table<RecordType extends DefaultRecordType>(props: TableProps<RecordType>) {
  ...
  const [supportOverlay, setSupportOverlay] = React.useState(false);
  ...
  if (fixHeader) {
    scrollYStyle = {
      overflowY: supportOverlay ? 'overlay' : 'scroll',
      maxHeight: scroll.y,
    };
  }

  if (horizonScroll) {
    scrollXStyle = { overflowX: supportOverlay ? 'overlay' : 'auto' };
    ...
    scrollTableStyle = {
      width: scroll?.x === true ? (supportOverlay ? 'overlay' : 'auto') : scroll?.x,
      minWidth: '100%',
    };
  }
  ...
  React.useEffect(() => {
    const canuseOverlay = isStyleSupport('overflow', 'overlay');
    setScrollbarSize(!canuseOverlay ? getTargetScrollBarSize(scrollBodyRef.current).width : 0);
    setSupportSticky(isStyleSupport('position', 'sticky'));
    setSupportOverlay(canuseOverlay);
  }, []);
