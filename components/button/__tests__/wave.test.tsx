...
describe('click wave effect', () => {
  async function clickButton(wrapper: any) {
    wrapper.find('.m-design-btn').getDOMNode().click();
    wrapper.find('.m-design-btn').getDOMNode().dispatchEvent(new Event('transitionstart'));
    await sleep(20);
    wrapper.find('.m-design-btn').getDOMNode().dispatchEvent(new Event('animationend'));
    await sleep(20);
  }
  ...
  it('should have click wave effect for primary button', async () => {
    ...
    expect(
      wrapper.find('.m-design-btn').getDOMNode().hasAttribute('m-design-click-animating-without-extra-node'),
    ).toBe(true);
  });
  ...
  it('should handle transitionstart', async () => {
    const wrapper = mount(<Button type="primary">button</Button>);
    await clickButton(wrapper);
    const buttonNode = wrapper.find('.m-design-btn').getDOMNode();
  ...
  it('should run resetEffect in transitionstart', async () => {
    ...
    wrapper.find('.m-design-btn').getDOMNode<HTMLButtonElement>().click();
    await sleep(10);
  ...
  it('should handle transitionend', async () => {
    ...
    wrapper.find('.m-design-btn').getDOMNode().dispatchEvent(event);
    expect(resetEffect).toHaveBeenCalledTimes(2);
    resetEffect.mockRestore();
  });
