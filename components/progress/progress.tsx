...
import CloseOutlined from '../_assets/CloseOutlined';
import CheckOutlined from '../_assets/CheckOutlined';
import CheckCircleFilled from '../_assets/CheckCircleFilled2';
import CloseCircleFilled from '../_assets/CloseCircleFilled2';
...
const ProgressStatuses = tuple('normal', 'exception', 'active', 'success', 'paused');
...
export default class Progress extends React.Component<ProgressProps> {
  ...
  renderProcessInfo(prefixCls: string, progressStatus: typeof ProgressStatuses[number]) {
    ...
    if (format || !['exception', 'success', 'paused'].includes(progressStatus) || progressStatus === 'paused' && !isLineType) {
      text = textFormatter(validProgress(percent), validProgress(successPercent));
    } else if (progressStatus === 'exception') {
      text = isLineType ? <CloseCircleFilled /> : <CloseOutlined />;
    } else if (progressStatus === 'success') {
      text = isLineType ? <CheckCircleFilled /> : <CheckOutlined />;
    } else if (progressStatus === 'paused' && isLineType) {
      text = 'Waiting'
    }
  ...
  renderProgress = ({ getPrefixCls, direction }: ConfigConsumerProps) => {
    ...
    const successPercent = getSuccessPercent(this.props);
    const hasSuccessPercent = successPercent !== undefined && !isNaN(successPercent) && successPercent >= 0;
    ...
    const classString = classNames(
      prefixCls,
      {
        ...
        [`${prefixCls}-success-percent`]: hasSuccessPercent,
      },
      className,
    );
