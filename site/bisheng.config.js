...
const realPkgName = '@m-design/mui'
...
module.exports = {
  ...
  lessConfig: {
    ...
    modifyVars: {
      'root-entry-name': ANT_THEME || DEV_THEME || 'default',
      'ant-prefix': process.env.ANT_PREFIX,
    },
  },
  webpackConfig(config) {
    config.resolve.alias = {
      [`${pkgName}/components/_assets`]: path.join(process.cwd(), 'components/_assets'),
      [`${pkgName}/lib`]: path.join(process.cwd(), 'components'),
      [`${pkgName}/es`]: path.join(process.cwd(), 'components'),
      [`${pkgName}`]: path.join(process.cwd(), 'site', 'antd'),
      [`${realPkgName}/components/_assets`]: path.join(process.cwd(), 'components/_assets'),
      [`${realPkgName}/lib`]: path.join(process.cwd(), 'components'),
      [`${realPkgName}/es`]: path.join(process.cwd(), 'components'),
      [`${realPkgName}`]: path.join(process.cwd(), 'site', 'antd'),
    ...
    delete config.module.rules[6].test;
    delete config.module.rules[6].loader;
    delete config.module.rules[6].options;
    config.module.rules[6].oneOf = [
      {
        test: /\.svg$/i,
        issuer: /\.(css|less|scss|styl)$/,
        use: [
          {
            loader: 'file-loader',
            options: {
