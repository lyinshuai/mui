...
const ComponentOverview = ({
  ...
  const documentTitle = `${title} - M Design`;
        ...
        .map(group => {
          ...
          return components.length ? (
            <div key={group.title} className="components-overview">
              ...
              {group.title === '重型组件' ? (
                <>
                  <Typography>如需使用pro系列，请别名安装@m-design/mui-ant包，可以确保pro组件依赖正确</Typography>
                  <Typography>yarn add antd@npm:@m-design/mui-ant@1.0.0 --registry=http://192.168.1.55:8081/repository/npm-all/</Typography>
                  <Typography>注意，mui-ant包样式前缀为ant，不是m-design</Typography>
                </>
              ) : null}
              <Row gutter={[24, 24]}>
                {components
